...
exports.FetchNFTFiListing = exports.AllNFTFiListingsForProject = exports.NFTFiListingsForProject = exports.VerifyNFTfiProjects = exports.VerifyNTFfiListings = exports.NFTFiProjects = exports.NFTFiTotalListings = void 0;
/**
 *
 ...
 *
 */
const NFTFiTotalListings = async () => {
...
exports.NFTFiProjects = NFTFiProjects;
/**
 *
 ...
 */
const VerifyNTFfiListings = (verified_projects, listings) => {
    return listings.filter((l) => verified_projects.map(p => p.address).includes(l._id));
};
exports.VerifyNTFfiListings = VerifyNTFfiListings;
/**
 *
 * @param projects An array of projects to filter
 * @returns An array of projects that have been verified by NFTFi
 */
const VerifyNFTfiProjects = (projects) => {
    return projects.filter(p => p.metadata.collection?.safelist_request_status == "verified");
};
exports.VerifyNFTfiProjects = VerifyNFTfiProjects;
/**
 *
 ...
 */
const NFTFiListingsForProject = async (address, skip = 0) => {
...
exports.NFTFiListingsForProject = NFTFiListingsForProject;
/**
 *
 * @param address The address of the NFTFi contract
 ...
 */
const AllNFTFiListingsForProject = async (address, max) => {
    let listings = [];
    let skip = 0;
    let done = false;
    listings = await Promise.all(new Array(Math.ceil(max / 20)).fill(null).map((_, i) => {
        return (0, exports.NFTFiListingsForProject)(address, i * 20);
    })).then(x => x.flat());
    return listings;
};
exports.AllNFTFiListingsForProject = AllNFTFiListingsForProject;
/**
 *
 ...
 */
const FetchNFTFiListing = async (contract, id) => {
    // fetch from NFTfi
...
exports.FetchNFTFiListing = FetchNFTFiListing;
