...
const NFTFi_1 = require("./utils/NFTFi");
const moment_1 = __importDefault(require("moment"));
main();
async function main() {
    // First get all the verified projects
    const projects = await (0, utils_1.NFTFiProjects)();
    const verified_projects = (0, NFTFi_1.VerifyNFTfiProjects)(projects);
    ...
    const listings = await (0, utils_1.NFTFiTotalListings)();
    const verified_project_listings = (0, utils_1.VerifyNTFfiListings)(verified_projects, listings);
    ...
    for (const project_listing of verified_project_listings) {
        x++;
        if (x % 10 == 0)
            console.log(`Checking listing ${x} of ${verified_project_listings.length}... (${(0, moment_1.default)().diff(started_at, 'seconds')}s)`);
        // Get detailed listings for this project
        const project_listings = await (0, NFTFi_1.AllNFTFiListingsForProject)(project_listing._id, project_listing.count);
        ...
        if (!lowestAskPricewETH)
            continue;
        // Get the opensea slug for the project
        const slug = await (0, utils_1.GetSlug)(project_listing._id);
        ...
        best_NFTFi_listings.push({
            ...
            timeSinceListed: lowestAskPricewETH.listedDate ? (0, moment_1.default)(lowestAskPricewETH.listedDate).fromNow() : undefined,
    ...
    console.log("\nDone scraping NFTFi! (took " + (0, moment_1.default)().diff(started_at, 'seconds') + "s)");
    ...
    const listings_with_desiredLoanPrincipalAmount_weth = best_NFTFi_listings.filter(x => {
